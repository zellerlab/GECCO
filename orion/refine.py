...
def AS_find_nr_cds(clusterpositions, seq_record):
...
def AS_is_good_cluster_hit(cdsnr, probabilities, pfam_ids, options):
...
def AS_find_cf_clusters(pfam_df, options):
    ...
    for n, row in pfam_df.iterrows():
        featurepositions = int(row["gene_start"]), int(row["gene_end"])
        try:
